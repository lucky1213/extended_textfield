...
class _SelectableTextSelectionGestureDetectorBuilder extends ExtendedTextSelectionGestureDetectorBuilder {
  _SelectableTextSelectionGestureDetectorBuilder({
    required ExtendedSelectableTextState state,
  })  : _state = state,
        super(delegate: state);

  final ExtendedSelectableTextState _state;
...
///
/// The [ExtendedSelectableText] widget displays a string of text with a single style.
...
/// {@end-tool}
///
/// Using the [ExtendedSelectableText.rich] constructor, the [ExtendedSelectableText] widget can
...
/// ## Interactivity
///
/// To make [ExtendedSelectableText] react to touch events, use callback [onTap] to achieve
...
class ExtendedSelectableText extends StatefulWidget {
  ...
  @override
  ExtendedSelectableTextState createState() => ExtendedSelectableTextState();
...
class ExtendedSelectableTextState extends State<ExtendedSelectableText>
  ...
  void initState() {
    ...
    if (widget.selectedAll) {
      Timer.periodic(Duration(milliseconds: 50), (t) {
        if (_editableText != null && textSelectionControls != null) {
          final RenderBox renderBox = context.findRenderObject() as RenderBox;
          final Offset offset = renderBox.localToGlobal(Offset.zero);
          final Rect selectionRect = Rect.fromLTWH(offset.dx, offset.dy, renderBox.size.width, renderBox.size.height);
          _editableText!.renderEditable.selectWordsInRange(from: selectionRect.topLeft, to: selectionRect.bottomRight, cause: SelectionChangedCause.doubleTap);
          //_editableText.bringIntoView(_editableText.textEditingValue.selection.extent);
          t.cancel();
        }
      });
    }
  ...
  @override
  void didUpdateWidget(ExtendedSelectableText oldWidget) {
  ...
  void _onControllerChanged() {
    // if (_selection != _controller.selection) {
  ...
  bool get allowFocus {
    if (_effectiveFocusNode is SelectableFocusNode) {
      return (_effectiveFocusNode as SelectableFocusNode).allowFocus;
    } else {
      return true;
    }
  }
